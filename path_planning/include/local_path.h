...
struct Carinfo
{
  ...
  double a_lat_max = 0.5; // 상황에 따라 수정
  double v_min = 0.0;     // m/s          // 상황에 따라 수정
  double v_max = 2.2;    // m/s         // 상황에 따라 수정
};
...
private:
  ...
  void generateCandidatePaths(Carinfo &car, const GlobalPathInfo *const global_path, vector<Obs> &obs, vector<pair<nav_msgs::Path, Pathinfo>> &candidate_paths);
  void generateLocalPath(Carinfo &car, double lane_offset, const GlobalPathInfo *const global_path, pair<nav_msgs::Path, Pathinfo> &path);
  double compute_delta_s_vel(double vel);
  ...
  void computeOptimalPath(Carinfo &car, const GlobalPathInfo *const global_path, vector<pair<nav_msgs::Path, Pathinfo>> &candidate_paths, nav_msgs::Path &optimal_path, double &target_v);
  double computeCurvatureVelocity(const pair<nav_msgs::Path, Pathinfo> &path_info);
