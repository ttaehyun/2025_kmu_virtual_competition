...
#include <std_msgs/Float32.h> // 추가: 속도와 각도 퍼블리시를 위한 메시지 타입
  ...
  use_servo_cmd_(true), publish_tf_(false), x_(0.0), y_(0.0), yaw_(0.0)
{
  ...
  // 추가: 현재 속도와 조향각 퍼블리셔 생성
  current_speed_pub_ = nh.advertise<std_msgs::Float32>("/current_speed", 10);
  current_angle_pub_ = nh.advertise<std_msgs::Float32>("/current_angle", 10);
...
void VescToOdom::vescStateCallback(const vesc_msgs::VescStateStamped::ConstPtr& state)
{
  ...
  current_speed_msg.data = current_speed; // m/s 단위
  current_speed_pub_.publish(current_speed_msg);

  std_msgs::Float64 current_angle_msg;
  current_angle_msg.data = current_steering_angle; // 라디안 단위
