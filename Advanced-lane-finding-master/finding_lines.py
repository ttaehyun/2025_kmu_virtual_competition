...
def rad_of_curvature(left_line, right_line):
    ...
    xm_per_pix = 3.7*(480/640) / width_lanes  # meters per pixel in x dimension
...
def blind_search(b_img, left_line, right_line):
    ...
    window_height = int(b_img.shape[0] / num_windows)
    ...
    # ⛑️ 예외 처리: 빈 벡터일 경우, 탐지를 건너뜀
    if len(leftx) == 0 or len(lefty) == 0 or len(rightx) == 0 or len(righty) == 0:
        left_line.detected = False
        right_line.detected = False
        print("[⚠️ 경고] 차선 점을 찾지 못함. blind_search 실패")
        return np.dstack((b_img, b_img, b_img)) * 255  # 빈 이미지 반환
...
def draw_lane(img, left_line, right_line, lane_color=(255, 0, 255), road_color=(0, 255, 0)):
    if left_line.allx is None or right_line.allx is None or left_line.ally is None:
        print("[⚠️ 경고] draw_lane: 차선 정보가 부족함")
        return img, img
...
def road_info(left_line, right_line):
    ...
    # 🔐 데이터 유효성 검사
    if (
        left_line.radius_of_curvature is None or right_line.radius_of_curvature is None or
        left_line.startx is None or right_line.startx is None or
        left_line.endx is None or right_line.endx is None
    ):
...
def print_road_map(image, left_line, right_line):
    ...
    if (
        left_line.startx is None or right_line.startx is None or
        left_line.allx is None or right_line.allx is None or
        left_line.ally is None or right_line.ally is None
    ):
        print("[⚠️ print_road_map] Missing lane data. Skipping rendering.")
        return np.zeros((95, 95, 3), dtype=np.uint8)
